...
# ======================================
# 고양이 캐릭터 렌더링 (사이드 패널용, 큰 이미지)
# ======================================
def render_cat_panel_large(text, mood="normal"):
    if mood == "angry":
        filename, emoji = "cat_angry.png", "😾"

    elif mood == "thinking":
        filename, emoji = "cat_thinking.png", "😿"

    else: # normal, happy
        filename, emoji = "cat_happy.png", "😺"
    ...
    if os.path.exists(img_path):
        ...
        # 이미지를 크게(width 100%) 표시
        img_tag = f'<img src="data:image/png;base64,{b64}" style="width: 100%; height: auto; filter: drop-shadow(0px 4px 6px rgba(0,0,0,0.1));">'
    else:
        img_tag = f'<div style="font-size: 100px;">{emoji}</div>'

    html_code = f"""
    <div class="cat-panel">
        <div style="margin-bottom: 10px;">{img_tag}</div>
        <div style="background-color: #ffffff; padding: 16px 22px; border-radius: 25px 25px 25px 3px; border: 2px solid #E3D5F5; box-shadow: 2px 2px 8px rgba(0,0,0,0.05); position: relative; bottom: 10px;">
...
if st.session_state.get("route_ready") and not st.session_state.get("is_loading"):
    ...
    # KPI 및 멘트 계산
    kpi = calculate_kpis(G, paths["shortest"], target_path if target_path else paths["cooling"], env_at_time["temp"], rain_mm, env_at_time["humidity"])
    grade_info = calculate_route_grade(kpi, mode=target_key if target_key else "cooling", pref=st.session_state.get("personal_pref"))
    ...
    if target_key:
        ...
        st.markdown(f"""
        <div style='background:#f8f9fa; border-radius:10px; padding:10px; margin-top:10px; text-align:center;'>
          <div style='font-size:12px; color:#888;'>선택 경로 등급</div>
          <div style='font-size:24px; font-weight:bold; color:#7E57C2;'>{grade} <span style='font-size:14px;'>({grade_info['score']}점)</span></div>
        </div>
        """, unsafe_allow_html=True)

    # ----------------------------------------------------
    # [레이아웃 핵심] 좌측(지도) : 우측(캐릭터) = 2.5 : 1
    # ----------------------------------------------------
    col_map, col_cat = st.columns([2, 1], gap="medium")
    ...
    with col_cat:
        ...
        if rain_mm > 0:
            comment = "비가 와서 그늘은 소용없어. ☔<br>대신 미끄러지지 않게 조심해서 걷자냥!"
            mood = "thinking"
        elif target_key == "personal":
             pk = st.session_state.get("selected_persona", "custom")
             if pk == "health": comment = "햇빛을 피해 제일 안전한 길로 골랐어.<br>건강이 최고니까!"
             else: comment, mood = "집사가 원한 딱 그 길이야!<br>얼른 가보자냥 🐾", "happy"

        if not comment:
            if grade == "A": comment, mood = "완벽해! 발바닥이 하나도 안 뜨거운<br>최고의 그늘길이야. 🐾", "happy"
            elif grade == "B": comment, mood = "나쁘지 않아.<br>적당히 시원하게 걸을 수 있겠어.", "normal"
            elif grade == "C": comment, mood = "음... 조금 애매한데?<br>땀이 좀 날 수도 있겠다.", "thinking"
            else: comment, mood = "으아, 여긴 털 탈 것 같아! 🔥<br>너무 뜨거우니까 다른 길로 가자.", "angry"

        # [NEW] 대형 캐릭터 패널 렌더링
        render_cat_panel_large(comment, mood)




    # ----------------------------------------------------
    # 3. 하단: KPI 상세 카드 및 그래프
    # ----------------------------------------------------
